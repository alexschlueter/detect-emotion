...
Ein weiteres Problem der Eingabedaten besteht darin, dass die Relation von true-positives zu true-negatives sehr gering ist, das heißt die Anzahl der Frames
...
Man kann jedoch eine statische Featureberechnung um einen zeitlichen Aspekt erweitern, indem wir ihre Veränderung über die Zeit als Feature benutzen (diese Methode bezeichnen wir als TimeDiff.)
...
\begin{itemize}
  ...
  \item Polynomieller Kernel: $K(x_i,x_j) = (yx_i^Tx_j + \theta)^{degree}$
...
Wir haben uns aber für Methode 2 entschieden, weil
...
Wir haben ein zweite Menge an Daten von 5 Personen, die als Testmenge.
...
Wir wollen in Kapitel \label{ch:ergebnis} die Ergebnisse vorstellen, die wie folgt Zustande gekommen sind:
Zuerst wird die Trainingsmenge und Validierungsmenge gebildet.
Die Videos der Trainingsmenge wird durch Rauschen künstlich erweitert und bei beiden Mengen die Landmarks normalisiert.
...
Schlussendlich wurden die Features für das Training dann zufällig geshuffled. Für zeitlichen Features haben wir uns nur auf X-/Y-Koordinaten und die Interpolationskoeffizienten konzentriert. Für die Ableitung wurde $\delta=2$ gesetzt.

Wir lernen dann mehrere SVM mit verschiedenen Kernelfunktionen und mehrere RF mit variierende  Anzahl an Bäumen und maximaler Tiefe für jeden der genannten Feature Extraction. Wir lernen auf der Trainingmenge und versuchen die Qualität des Klassifikator-Feature Paares für jede Action Unit anhand der Validierungsmenge und des dortige F1 score zu beurteilen.
Da pro Action Unit ca. 160
